...
**不是像素。** 智能体看不到 Foxglove 的界面，也没有对录制内容的视频理解能力。它
关于 Foxglove 所看到的，只是一小组带类型的能力及其 JSON 应答——设备 id、录制 id、
字节数、事件 id。

*空间*层面的理解来自完全另一处：`inspect_mcap_recording` 工具在服务端解析 ROS2
bag，返回实测数值——各话题消息数、各轴 TF 平移增量、代价地图障碍距离、相机帧率。
智能体是在这些测量值之上做推理的。

Foxglove 在这个循环里的作用，是把同样的证据变成**人类**能够判断的形式：真实的
bag、时间线，以及在智能体标记的那一刻打上的标注。智能体负责测量；Foxglove 是它
告诉机械臂操作员"该看哪里"的方式。
