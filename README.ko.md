...
**픽셀은 아닙니다.** 에이전트는 Foxglove UI를 볼 수 없고, 레코딩에 대한 영상 이해
능력도 없습니다. 에이전트가 Foxglove에 대해 보는 것은 소수의 타입 지정된 능력과
그 JSON 응답 — 디바이스 id, 레코딩 id, 바이트 수, 이벤트 id — 뿐입니다.

_공간적_ 이해는 완전히 다른 곳에서 옵니다. `inspect_mcap_recording` 도구가 서버
사이드에서 ROS2 bag을 파싱해 측정값을 반환합니다: 토픽별 메시지 수, 축별 TF
이동량 델타, 코스트맵 장애물 거리, 카메라 프레임레이트. 에이전트는 그 측정값 위에서
추론합니다.

루프에서 Foxglove의 역할은 같은 증거를 **사람**이 판단할 수 있는 형태로 만드는
것입니다. 실제 bag, 타임라인, 에이전트가 지적한 순간에 붙은 주석. 에이전트가
측정하고, Foxglove는 리그 오퍼레이터에게 어디를 봐야 하는지 보여주는 방법입니다.
