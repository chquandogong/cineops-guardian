...
ENTRIES = [
    ("INFO",  "dolly-rig-mgr",        "Lens swap acknowledged: CinePrime 35mm T1.5 mounted on Camera Dolly Alpha."),
    ("WARN",  "tf2_ros_broadcaster",  "Static transform checksum mismatch for [camera_optical_frame] -> [lidar_link]. Expected CRC 0x8F4A, active 0x3E12."),
    ("WARN",  "costmap_2d",           "Rapid obstacle inflation detected near coordinates (x: 4.82, y: 2.15). Clearance threshold violated (1.25m < 1.50m)."),
    ("ERROR", "nav2_controller",      "Navigation recovery behavior entered: [SpinRecovery -> BackupRecovery]. Consecutive retry count: 7."),
    ("ERROR", "camera_streamer",      "Frame delivery degradation: 32.5% frames dropped in last 5000ms. Optical genlock jitter exceeded 18ms."),
]
...
req = urllib.request.Request(
    LOKI_URL, data=body, method="POST",
